#!/usr/bin/env python3
###отключена возможность выйти из состояния aiming###
import math
...
class FSM(object):
    def __init__(self):
        ...
        self.dest_x = None
        self.dest_y = None
        self.camera_angle = 0.   # угол поворота камеры в системе координат робота
        ...
        self.dest_sub = rospy.Subscriber('/destination', Point, self.cb_dest)
    ...
    def cam_pid(self, err):
        ...
        #rospy.logerr("err: {:.6f}; p: {:.6f}; i: {:.6f}; d: {:.6f}".format(err, pp, ii, dd))
        signal = pp + ii + dd
        return signal
    ...
    #относительное направление на цель
        return angle_diff(math.atan2(self.dest_y, self.dest_x) + self.camera_angle, 0)
        #угол в поле зрения камеры + угол поворота камеры
    ...
    #действие в соответствии с состоянием
        ...
        #Вращать основание. Камера смотрит прямо на цель
            ...
            try:
                self.cam_pos_msg.data = self.camera_angle + self.cam_pid(math.atan2(self.dest_y, self.dest_x))
    ...
    #True, если направление камеры на цель примерно ноль
        if abs(math.atan2(self.dest_y, self.dest_x)) < self.SMALL_ANGLE:
    ...
    #цель условно достигнута
        if math.hypot(self.dest_x, self.dest_y) < 0.2:
            return True
        else:
            return False
